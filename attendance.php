    ...
    if ($query->num_rows > 0) {
        ...
        if ($status == 'in') {
            ...
            if ($query->num_rows > 0) {
                ...
                //
                $tim = mt_rand(0, 1000);
                $sql = "INSERT INTO attendance (employee_id, date, time_in,time_out, status,num_hr) VALUES ('$id', '$date_now', NOW(), '$logstatus','00:00:00',$tim)";
                if ($conn->query($sql)) {
                    $output['message'] =
                        'Time in: ' .
                        $row['firstname'] .
            ...
            if ($query->num_rows < 1) {
                ...
                if ($row['time_out'] != '00:00:00') {
                    ...
                    if ($conn->query($sql)) {
                        ...
                        $sql =
                            "UPDATE attendance SET num_hr = '$int' WHERE id = '" .
                            $row['uid'] .
                            "'";
                        $conn->query($sql);
                    } else {
